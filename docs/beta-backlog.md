...
- **DECISIONS (owner, 2026-06-30):**
  ...
  - **A1 — DECIDED (research-backed):** evidence is conclusive — JetBrains' own guide says "avoid data classes in your API"; the closest analogues (OpenAI/Anthropic Java-Kotlin SDKs) use non-data immutable classes + builders; AWS/OkHttp/AndroidX agree. **Approach: `@Poko` compiler plugin** (value semantics minus copy()/componentN(), KMP-safe). **Scope: audit ALL 358 public data classes case-by-case** → classify DEMOTE-to-@Poko (growable read-only results/responses/metadata) / KEEP-data (frozen small value types) / BUILDER-front (growable construct types). Plan: (1) classification manifest → (2) add Poko plugin → (3) batched demotion with ABI regen + tests. Sources in `docs/data-class-audit.md`.
- **Acceptance criteria (remaining mechanical changes):**
  - [x] **A1 [HIGH] Data-class migration** — manifest complete in `docs/data-class-audit.md` (378 classified): **178 DEMOTE→@Poko**, 139 BUILDER-front, 34 KEEP, 10 REVIEW, 17 non-public gate artifacts. Poko plugin wired (✓ `1cc9ed1`). **Key risk: 77 DEMOTE types are `@Serializable`; the @Poko+@Serializable combo is unproven in-repo** → first task is a minimal canary proving the combo (1 type + JSON round-trip + checkKotlinAbi) BEFORE the batches. Then 14 file-cohesive DEMOTE batches (D1 streaming-events canary … D14 errors/telemetry), each = one ABI regen + ratchet the data-class budget down. 58 DEMOTE types have internal `copy()` to rewrite as fresh-construct. BUILDER track (139) is separate/heavier (hand-written builders). REVIEW set (10) decided case-by-case (LanguageModelCallParams etc.). Completed in BL-058 A1: public data-class budget ratcheted from 378 to 66 with DEMOTE, BUILDER, and KEEP-floor audit complete.
