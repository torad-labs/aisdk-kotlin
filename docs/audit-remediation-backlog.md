...
## Tracked debt from review 4813532855

| ID | Task | Status |
|----|------|--------|
| AR-37 | Burn down the complexity entries this PR baselined instead of fixing. `detekt-baseline-budget.json` `raiseRationale` marks them as new debt, not accepted: `CyclomaticComplexMethod` +14 and `LongMethod` +10 relative to `main`. The new entries cluster in provider wire-mapping written by `0ce4de3`: `AnthropicRequestMapping.kt` (`PreparedAnthropicRequest`, `AnthropicAssistantPart`, `AnthropicUserPart`, `AnthropicPrompt`), `AnthropicTools.kt` (`AnthropicPrepareTools`, `AnthropicProviderExecutedTool`), `OpenResponsesRequestMapping.kt` (`OpenResponsesToolChoice`, `OpenResponsesProviderToolJson`, `OpenResponsesCodeInterpreterContainer`), `BedrockRequest.kt`, `GoogleInteractionsMapping.kt`, `GoogleLanguageModel.kt`, `AlibabaProvider.kt`, `BlackForestLabsProvider.kt`, `ReplicateProvider.kt`, `SchemaSanitizer.kt`, `CohereStreamState.kt`, `LiteRTLanguageModel.kt`, `MessageStreamReader.kt`, `XaiMediaModels.kt`. Deliberately NOT done in this PR: splitting ~20 wire-mapping functions is a behaviour-risking refactor on paths CI currently proves green, and it is orthogonal to the review's other findings — landing it in the same change would make both harder to review. One entry WAS burned down rather than baselined (`VoyageProvider.voyageEmbeddingRepresentation`), which is why the ceiling is 70 and not 71. Accept: both ceilings at or below `main`'s (56 / 157), `./gradlew check` green, and no provider wire assertions changed. | TODO |
| AR-39 | Make an unpoliced off-origin download not COMPILE, rather than not pass a gate. The Fireworks credential leak and its four failed fixes all shared one shape: an ordinary `HttpClient` is in scope, so forwarding caller headers to a provider-supplied URL is the path of least resistance and every wall against it is a detector looking for that shape after the fact. The structural version is a `PolicedClient` wrapper — a client that can only be obtained by declaring the origin policy, with the raw `HttpClient` unreachable from the download helpers — so the leak becomes unrepresentable instead of detectable. Deliberately NOT done in this PR: it re-plumbs transport construction across 45 providers, which is a behaviour-risking refactor on paths CI currently proves green, and the two mechanical walls that exist (`detect-provider-asset-credential-leak.py`, keyed per SITE and fail-closed, plus `no-uncapped-response-body` LAW) cover the known instances today. Accept: every provider download obtains its client through the policed type, the detector's allowlist drops to zero entries because the shape it looks for no longer exists, and `./gradlew check` stays green. | TODO |
| AR-38 | `no-public-without-since` still warns on ~26 sites where a long KDoc block is stacked directly above the `/** @since */` line: ast-grep's `follows` does not see the marker there, and every formulation that closes it (`stopBy: end`, a `stopBy` rule, nested `follows`) over-matches and exempts the whole tree at 0 hits — silent false negatives instead of noise. The rule's `note` records the measurement. The blocking owner (`tools/check-public-api-since-budget.mjs`, 12-line lookback) handles the shape correctly and reports 0 missing, so this is edit-time noise, not a coverage gap. Accept: either an ast-grep formulation that keeps the 143 true positives while exempting the stacked-comment shape, or a decision to drop the ast-grep mirror and let the Node gate own the invariant alone. | TODO |
