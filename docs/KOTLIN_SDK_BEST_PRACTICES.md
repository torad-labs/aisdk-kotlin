...
3. **Gate the public surface on the committed ABI dump.** `checkKotlinAbi` runs under `check` and fails on any unreviewed change to the JVM ABI (`api/jvm/aisdk-kotlin.api`, 13274 lines) or the merged klib ABI (`api/aisdk-kotlin.klib.api`, 13833 lines). Treat a dump diff as a reviewable API change, never a formality. *Exemplar:* `build.gradle.kts:56` (`abiValidation { ... }`), `keepLocallyUnsupportedTargets.set(true)` lets Linux CI infer the *iOS* klib ABI (the locally-buildable `linuxX64` ABI is emitted for real). ([KGP ABI validation](https://kotlinlang.org/docs/gradle-binary-compatibility-validation.html))
...
9. **Evolve, don't remove — and tie the deprecation ladder to versions.** Retire API with `@Deprecated(message, replaceWith = ReplaceWith(...))` escalating `WARNING` → `ERROR` → `HIDDEN`, never removal without a cycle. The three levels map to compatibility: `WARNING` is source-and-binary-safe (the call still compiles); `ERROR` is source-breaking but still generates the old bytecode, so already-compiled clients keep linking (binary-safe); `HIDDEN` removes it from source resolution while *still* emitting the bytecode, so binary compatibility survives until the symbol is finally deleted in a major. Don't bump the levels by editing the annotation on each release by hand — pin them to API versions with `@DeprecatedSinceKotlin(warningSince, errorSince, hiddenSince)` (or the library's own equivalent), which derives the diagnostic severity from the consumer's version so the ladder advances mechanically. Do **not** abuse `@RequiresOptIn` to retire existing declarations — opt-in is for staging new surface, not killing old. ([deprecation cycle](https://kotlinlang.org/docs/api-guidelines-backward-compatibility.html), [`@DeprecatedSinceKotlin`](https://kotlinlang.org/api/core/kotlin-stdlib/kotlin/-deprecated-since-kotlin/))
...
11. **Tie release numbers to SemVer and back it with `@since` + a kept CHANGELOG.** Once 1.0 is cut, the version number is the compatibility contract: a binary-or-source break is a **major** bump, additive public surface is a **minor**, and a fix with no surface change is a **patch** — and the gate that proves the claim is the ABI dump (§1.3), not a reviewer's memory. Encode "when did this appear" in the source: every new public declaration carries a `@since <version>` KDoc tag (the standard tag for "version this element was introduced"), so consumers and the deprecation ladder (§1.9) both have a version anchor. And every reviewable ABI-dump change ships with a `CHANGELOG.md` entry in the same PR — `CHANGELOG.md` exists and declares "follows Semantic Versioning once the first stable release is cut" (`CHANGELOG.md:5`), so the discipline is committed, not aspirational. *Gap:* there is **zero** `@since` usage in the tree today (`grep -rn '@since' src` is empty) and the project is pre-1.0 (`0.1.0-SNAPSHOT`), so the policy is declared but not yet exercised — start tagging now so the 1.0 surface is fully annotated. No static check enforces the *correctness* of a bump (that it matches the ABI delta) — this is **manual review backed by the `checkKotlinAbi` diff**: a dump change with only a patch bump, or any disappearance without a prior `@Deprecated` release, is a finding. ([SemVer dimensions](https://kotlinlang.org/docs/api-guidelines-backward-compatibility.html), [`@since` KDoc tag](https://kotlinlang.org/docs/kotlin-doc.html))
...
6. **Put `actual` implementations in the highest shared intermediate set.** For `iosArm64` + `iosSimulatorArm64`, the single `actual` goes in `iosMain`; leaf sets stay empty unless a leaf genuinely differs. No copy-pasted device/simulator `actual`s. When an `actual` is shared across *all* Native targets (Apple + Linux), it goes one level higher still — *exemplar:* the stdio MCP `actual` lives in `src/nativeMain/.../MCPStdioProcess.native.kt`, inherited by the iOS targets and `linuxX64` alike, not duplicated per family.
...
8. **Build all targets in CI from day one — and run a Native target you *can* run cheaply.** A single-target project lets common code reach that target's symbols; a second target makes the leak a compile error at the worst time. A Native bug is worse: it may compile everywhere and only fail at *runtime* on Native. Apple targets need `macos-latest` to link+run, so put them in CI (*exemplar:* the `verify-apple` leg builds + tests `iosSimulatorArm64`, `.github/workflows/ci.yml`) — but also run a Native target that executes on the cheap leg, because the Apple runtime is slow and costly to gate on. *Exemplar:* `linuxX64` compiles **and runs** on `ubuntu-latest`, so `linuxX64Test` runs inside the ubuntu `check`; it has already caught a Flow emission-context violation (Native-only, green on JVM) and two unsynchronized test-double `ArrayList`s racing under the multi-threaded Native dispatcher — all invisible to the JVM/iOS single-threaded test runtimes.
...
10. **Put runnable examples in compiled sample functions referenced by `@sample`, never in fenced code blocks inside KDoc.** A fenced ```` ```kotlin ```` block in a doc comment is never compiled, so it rots silently the first time a signature changes — the doc then ships a snippet that doesn't compile. The `@sample fully.qualified.functionName` tag inlines the *body of a real function* into the rendered docs, and that function lives in a source set the build actually compiles, so a breaking change to the API breaks the sample's compile. Wire it: in the Dokka block (`build.gradle.kts:169`, Dokka 2.2.0 already applied) add `dokkaSourceSets { configureEach { samples.from("samples/…") } }`, point it at a compiled `samples`/test source set, and reference functions with `@sample` (the tag must come *after* the prose — Dokka can't interleave samples and text). *Gap:* there is **zero** `@sample` usage and no `samples` source set today (`grep -rn '@sample' src` is empty), so any example currently must be a code fence by default — adopt the source set before writing the first example. ([KDoc tags](https://kotlinlang.org/docs/kotlin-doc.html), [Dokka samples](https://kotlinlang.org/docs/dokka-gradle-configuration-options.html))
...
## 10. Security, Supply Chain & Observability

This SDK holds provider API keys and shuttles prompts/responses that can echo secrets back. `SECURITY.md:7` makes the obligation explicit: "Security-sensitive provider integrations should document their own secret handling, retry, **logging, and redaction** behavior." This section is where that obligation becomes a reviewable rule.

1. **Never surface a raw response body or response headers without a redaction seam.** LLM error payloads routinely echo the request — including the prompt and, on a misconfigured gateway, the `Authorization` header or key — straight back in the body. `APICallError` carries `responseBody: String?` and `responseHeaders: Map<String, String>?` raw (`AiSdkError.kt:16`), and the gateway transport passes `responseBody = response.value` and the flattened header map through untouched (`KtorGatewayTransport.kt:53`; `HttpTransport.kt:39` flattens *every* response header into the carried map). That is the correct data to *carry* for retry classification (§9.3) — but the moment it is logged, attached to a crash report, or rendered to a user it must pass through a redaction step that strips `Authorization`/`api-key`/`x-api-key` headers and masks bearer-token-shaped substrings. **No static check proves a body was redacted before egress** — boxing-style invisible at the type level — so this is **manual review backed by a grep for the egress sites**: any code that logs or rethrows `responseBody`/`responseHeaders` outside the typed `APICallError` carrier is a finding. (`grep -rnE 'responseBody|responseHeaders' src` enumerates the carriers to audit.)

2. **Route all diagnostics through the injected `Logger` seam; never `println`/`NSLog`/`Log.d` in library code.** A published library must not hard-code a platform log target — it can't know whether the host wants Logcat, `os_log`, or a structured server sink. The seam already exists: `Logger` with `warn`/`info`/`debug` and a `NoopLogger` default (`Logger.kt:28`), consumed by `loggingMiddleware` (`Logging.kt:34`). Keep new diagnostics on that interface. *Exemplar + caveat in one:* `loggingMiddleware` logs full tool-call arguments at `debug` (`args=${event.inputJson}`, `Logging.kt:55`) — that is prompt-adjacent data and must stay on `debug` (which production hosts gate off) and be covered by rule §10.4's no-content-by-default stance; never promote argument/content logging to `info`/`warn`. `grep -rnE 'println\(|NSLog\(|Log\.[dewiv]\(' src` must stay empty in non-test main source (it currently is).

3. **A tracing/metrics hook follows the OpenTelemetry GenAI semantic conventions — don't invent ad-hoc span/metric names.** For an LLM client the cross-vendor standard is OTel's GenAI semconv: one **client span** per model request (span kind `CLIENT`) with `gen_ai.operation.name`, `gen_ai.provider.name`, `gen_ai.request.model`, `gen_ai.response.model`, `gen_ai.response.finish_reasons`; and the metric `gen_ai.client.token.usage` (a histogram, dimensioned by `gen_ai.token.type` = `input`/`output` — note the *metric* tags the type, it is **not** two metrics named `…input_tokens`/`…output_tokens`) plus `gen_ai.client.operation.duration`. These conventions are still **Development/experimental** as of 2026 (client spans/metrics are stable in practice and vendor-supported, but the spec is not formally marked stable), so any hook ships behind `@ExperimentalAiSdkApi` (§8.4) and uses `OTEL_SEMCONV_STABILITY_OPT_IN` semantics for version transitions rather than freezing attribute names into the ABI. *Gap:* there is no OTel hook today — the only observability primitive is the `Logger`/`loggingMiddleware` pair (§10.2); a span-per-request + token-usage metric is an additive, middleware-shaped feature, not a core change. ([GenAI semconv](https://opentelemetry.io/docs/specs/semconv/gen-ai/), [GenAI metrics](https://opentelemetry.io/docs/specs/semconv/gen-ai/gen-ai-metrics/))

4. **No prompt/response content in telemetry by default; opt-in only.** Token *counts*, durations, model ids, and finish reasons are safe dimensions; the prompt and completion text are not. A tracing hook must default to recording metadata only and require an explicit, documented opt-in (matching OTel's content-capture being off by default) before any message body enters a span or log. This is the telemetry-side restatement of §10.1: the redaction obligation doesn't stop at error bodies — it covers every PII-bearing field on the happy path too. No grep proves "no content captured"; **manual review of every span/log attribute** against an allowlist of metadata fields is the control.

5. **Pin and verify the dependency supply chain with `gradle/verification-metadata.xml`.** Artifact signing is configured for *outbound* publication (`build.gradle.kts:233`, `sign(publishing.publications)`), but nothing verifies the *inbound* dependencies the build pulls. Gradle's dependency verification fails the build when a downloaded artifact's SHA-256 (and optionally PGP signature) doesn't match a committed expectation in `gradle/verification-metadata.xml` — the defense against a compromised or namespace-hijacked dependency. Bootstrap it with `./gradlew --write-verification-metadata sha256,pgp --export-keys`, then **review the generated file by hand** (bootstrapping trusts whatever is currently resolvable, so it will happily record an already-compromised artifact) and commit it; it auto-activates the moment it exists at that fixed path. Pin versions in `libs.versions.toml` rather than relying on dynamic/`+`/`-SNAPSHOT` coordinates, which sidestep the checksum. *Gap:* `gradle/verification-metadata.xml` does **not** exist today (`gradle/` holds only `libs.versions.toml` + `wrapper/`), so inbound integrity is unverified — adding it is the single highest-leverage supply-chain step. Detect the absence directly: `test -f gradle/verification-metadata.xml`. ([Gradle dependency verification](https://docs.gradle.org/current/userguide/dependency_verification.html))

6. **Own resource lifecycle explicitly; expose `close()`/`AutoCloseable` and never rely on finalizers.** A streaming client holds sockets, channels, and coroutine scopes that must be released deterministically. The repo already does the call-scoped half: `ToolLoopAgent.close()` cancels its owned scope (`ToolLoopAgent.kt:132`) and the MCP transports expose `suspend fun close()` (`MCP.kt:175`). On the JVM, types whose lifetime the consumer manages should implement `AutoCloseable` (Stable and multiplatform since Kotlin 2.0) so callers get `use { }` and try-with-resources; an injected `HttpClient` (caller-owned, `HttpTransport.kt:93`) is *not* closed by the SDK — document that ownership boundary so neither side double-closes or leaks. Make `close()` idempotent (a second call is a no-op). On Kotlin/Native the new memory model means **no `freeze()` and no `@SharedImmutable`** — shared mutable state uses `AtomicReference`/`@Volatile`/atomics (§6.9), never the legacy freezing APIs; a `freeze()` call or `ensureNeverFrozen` in new code is a finding. ([`AutoCloseable` stable in 2.0](https://kotlinlang.org/api/core/kotlin-stdlib/kotlin/-auto-closeable.html), [Kotlin/Native FAQ](https://kotlinlang.org/docs/native-faq.html))
...
| Public-ABI change (new surface / break) shipped without a matching SemVer bump or CHANGELOG entry | The version number is the compatibility contract; a silent bump-mismatch lies to consumers' resolvers | Bump per SemVer (break=major, additive=minor, fix=patch); add a `CHANGELOG.md` entry in the same PR | **No static bump-correctness check** — manual review backed by the `checkKotlinAbi` diff: any dump change without a CHANGELOG entry, or a break under a non-major bump, is a finding |
| New public declaration with no `@since <version>` KDoc tag | No version anchor for consumers or the deprecation ladder | `@since` on every new public member from now to 1.0 | `grep -rn '@since' src` is currently empty; review: each new public decl in the diff carries `@since` |
...
| Fenced ```` ```kotlin ```` code block in a KDoc comment instead of `@sample` | Never compiled → rots silently; ships a snippet that no longer compiles after a signature change | Compiled sample function referenced by `@sample`, wired via `dokkaSourceSets { samples.from(...) }` | `grep -rn '```' src/commonMain` finds doc-comment fences to convert; `grep -rn '@sample' src` (currently empty) tracks adoption |
...
### Security, Supply Chain & Observability

| Smell | Why rejected | Do instead | How to detect |
|---|---|---|---|
| Logging / surfacing raw `responseBody`/`responseHeaders` (or an `Authorization`/key header) without redaction | Echoes the prompt and, on a misconfigured gateway, secrets/keys into logs, crash reports, or UI | Redact at every egress — strip auth/`api-key` headers, mask bearer-token-shaped substrings | **No static proof of redaction** — review + `grep -rnE 'responseBody\|responseHeaders' src` to audit each egress site outside the typed carrier |
| `println` / `NSLog` / `Log.d` in main library code | Hard-codes a platform log sink a published library can't choose for the host | Inject the `Logger` seam (`NoopLogger` default) | `grep -rnE 'println\(\|NSLog\(\|Log\.[dewiv]\(' src/commonMain src/*Main` must stay empty |
| Ad-hoc span/metric names for the LLM tracing hook | Not portable across OTel tooling; freezes vendor-specific names | OTel GenAI semconv (`gen_ai.*`, `gen_ai.client.token.usage` histogram), gated `@ExperimentalAiSdkApi` | Review telemetry attribute/metric names against the GenAI semconv |
| Prompt/response **content** in spans or logs by default | Leaks PII/secrets into telemetry on the happy path | Metadata-only (counts, durations, model id, finish reason) by default; explicit opt-in for content | Review every span/log attribute against a metadata allowlist (no static check) |
| Build resolves dependencies with no `gradle/verification-metadata.xml` | Inbound supply chain unverified — a hijacked/compromised artifact runs in CI and consumers | Commit a hand-reviewed `verification-metadata.xml`; pin versions, no dynamic/`+`/SNAPSHOT coordinates | `test -f gradle/verification-metadata.xml`; grep `libs.versions.toml` for `+`/dynamic versions |
| Consumer-managed resource with no `close()`/`AutoCloseable`; `freeze()`/`@SharedImmutable` in new code | Leaked sockets/scopes; the legacy Kotlin/Native freezing model is gone | `AutoCloseable` + idempotent `close()`; atomics (§6.9) for shared state | `grep -rnE 'freeze\(\)\|@SharedImmutable\|ensureNeverFrozen' src` empty; review lifecycle types for `close()` |
...
- [ ] New public declarations carry `@since`; an ABI-dump change ships with a `CHANGELOG.md` entry and a SemVer-correct bump; runnable examples use `@sample`, not KDoc code fences.

**Security / Observability**
- [ ] No raw `responseBody`/`responseHeaders`/auth header logged or surfaced without redaction; no prompt/response content in telemetry by default.
- [ ] No `println`/`NSLog`/`Log.d` in main source — diagnostics go through the injected `Logger`; any tracing hook uses OTel GenAI semconv names and is `@ExperimentalAiSdkApi`-gated.
- [ ] `gradle/verification-metadata.xml` present (or its absence tracked); dependency versions pinned, no dynamic/SNAPSHOT.
- [ ] Consumer-managed resources are `AutoCloseable` with an idempotent `close()`; no `freeze()`/`@SharedImmutable` in new Kotlin/Native code.
...
- **No `wasmJs`/`js` target yet.** For a port of a JS-native SDK the browser is the most defensible next target and the streaming/Flow core is platform-agnostic; it's a roadmap item, not a regression. Current targets: `jvm`/`android`/`ios`/`linuxX64` — the Linux/Native target runs the Kotlin/Native test suite on the cheap CI leg and locally (§2.8), and ships an `aisdk-kotlin-linuxx64` artifact for server-side Kotlin/Native + CLI consumers.
...
- **Coverage measured, not enforced** (§7.9); **detekt baseline (~3120 entries)** ratchets new code only. Both are tracked debt with a burn-down commitment, **not** accepted steady state — the baseline is a migration scaffold (§7.4), and a release that lets the count climb is itself a finding.
