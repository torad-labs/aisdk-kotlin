...
internal fun awsSigV4SignedHeaders(
    ...
    val signature = hmacSha256Digest(signingKey, stringToSign.encodeToByteArray()).toHex()
...
private fun awsSigV4SigningKey(secretAccessKey: String, date: String, region: String, service: String): ByteArray {
    val dateKey = hmacSha256Digest("AWS4$secretAccessKey".encodeToByteArray(), date.encodeToByteArray())
    val dateRegionKey = hmacSha256Digest(dateKey, region.encodeToByteArray())
    val dateRegionServiceKey = hmacSha256Digest(dateRegionKey, service.encodeToByteArray())
    return hmacSha256Digest(dateRegionServiceKey, "aws4_request".encodeToByteArray())
}

private fun hmacSha256Digest(key: ByteArray, message: ByteArray): ByteArray {
    val blockSize = 64
    val normalizedKey = when {
        key.size > blockSize -> sha256Digest(key)
    ...
    return sha256Digest(outer + sha256Digest(inner + message))
}

private fun sha256Hex(input: ByteArray): String = sha256Digest(input).toHex()
