 ...
 * implementation derives from kotlinx.serialization descriptors; this
 * stub returns a permissive schema so the contract compiles before the
 * descriptor walker lands. See docs/AISDK_PORT_DECISIONS.md decision 2.
 */
...
private fun descriptorToJsonSchema(
    ...
    return when (kind) {
        ...
        -> objectSchema(descriptor)
        kotlinx.serialization.descriptors.StructureKind.LIST -> listSchema(descriptor)
        kotlinx.serialization.descriptors.StructureKind.MAP -> mapSchema()
...
private fun objectSchema(
    ...
    for (i in 0 until descriptor.elementsCount) {
        ...
        properties[name] = descriptorToJsonSchema(descriptor.getElementDescriptor(i))
...
private fun listSchema(
    ...
    val items = if (descriptor.elementsCount > 0) {
        descriptorToJsonSchema(descriptor.getElementDescriptor(0))
    } else {
...
private fun mapSchema(): kotlinx.serialization.json.JsonObject = jsonObj(
    SCHEMA_TYPE_KEY to kotlinx.serialization.json.JsonPrimitive(SCHEMA_OBJECT),
