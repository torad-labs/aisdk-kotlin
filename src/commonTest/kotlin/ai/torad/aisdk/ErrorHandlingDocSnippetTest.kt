    ...
    fun `streaming errors snippet compiles and runs`() = runTest {
        ...
        agent.stream(prompt = prompt, options = context).collect { event ->
            when (event) {
                ...
                is StreamEvent.StreamStart -> renderEvent(event)
                is StreamEvent.ResponseMetadata -> renderEvent(event)
                is StreamEvent.StepStart -> renderEvent(event)
                is StreamEvent.TextStart -> renderEvent(event)
                is StreamEvent.TextDelta -> renderEvent(event)
                is StreamEvent.TextEnd -> renderEvent(event)
                is StreamEvent.ReasoningStart -> renderEvent(event)
                is StreamEvent.ReasoningDelta -> renderEvent(event)
                is StreamEvent.ReasoningEnd -> renderEvent(event)
                is StreamEvent.SourcePart -> renderEvent(event)
                is StreamEvent.FilePart -> renderEvent(event)
                is StreamEvent.Data -> renderEvent(event)
                is StreamEvent.ToolInputStart -> renderEvent(event)
                is StreamEvent.ToolInputDelta -> renderEvent(event)
                is StreamEvent.ToolInputEnd -> renderEvent(event)
                is StreamEvent.ToolCall -> renderEvent(event)
                is StreamEvent.ToolResult -> renderEvent(event)
                is StreamEvent.ToolApprovalRequest -> renderEvent(event)
                is StreamEvent.ToolOutputDenied -> renderEvent(event)
                is StreamEvent.StepFinish -> renderEvent(event)
                is StreamEvent.Abort -> renderEvent(event)
                is StreamEvent.Raw -> renderEvent(event)
            }
        }
