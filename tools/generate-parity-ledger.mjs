...
const packageStatus = new Map([
  ...
  ['@ai-sdk/anthropic-aws', 'in-progress: createAnthropicAws/anthropicAws, AnthropicAwsProviderSettings, AnthropicAwsCredentials alias, AWS-hosted Anthropic base URL mapping, workspace header, API-key auth, Anthropic Messages generate/stream reuse, Anthropic hosted tool descriptors, unsupported embedding/image errors, and user-agent behavior are represented as a Kotlin facade folded into the root module; native SigV4 signing remains an open shared AWS infrastructure gap; VERSION is exposed as ANTHROPIC_AWS_VERSION until package modules are split'],
  ...
  ['@ai-sdk/openai', 'in-progress: createOpenAI/openai facade, hosted OpenAI tool descriptors, default Responses API model routing, OpenAI Responses request option mapping, supported URL metadata, built-in Responses tool type mapping, Responses metadata/logprob output mapping, OpenAI/Azure file-id prefix handling, and fake HTTP tests are folded into the root module; provider-tool argument factories remain open; VERSION is exposed as VERSION until package modules are split'],
