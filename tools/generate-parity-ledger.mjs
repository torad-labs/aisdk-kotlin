...
const referenceVersion = process.env.AI_SDK_REFERENCE_VERSION ?? '6.0.208';
...
const packageStatus = new Map([
  ...
  ['@ai-sdk/alibaba', 'ported: createAlibaba/alibaba, AlibabaProviderSettings, Alibaba chat aliases/options, cache-control metadata and usage parsing, Alibaba OpenAI-compatible chat routing, Qwen thinking option mapping, parallel tool-call option mapping, cache-write usage correction, reasoning/tool-call parsing via the shared chat adapter, DashScope text-embedding (text-embedding-v3/v4) embedding model with text-type/dimension/output-type options and sparse-output rejection, DashScope video task creation/polling, T2V/I2V/R2V input mapping, video option mapping, URL video outputs, warnings, error handling, and provider metadata are represented as a Kotlin facade folded into the root module; VERSION is exposed as ALIBABA_VERSION until package modules are split'],
  ...
  ['@ai-sdk/deepseek', 'ported: createDeepSeek/deepseek, provider settings, language options, error response handling, and chat model routing are represented as an OpenAI-compatible Kotlin facade folded into the root module; VERSION is exposed as DEEPSEEK_VERSION until package modules are split'],
  ...
  ['@ai-sdk/langchain', 'not-ported: the LangChain JS bridge symbols toBaseMessages, convertModelMessages, StreamCallbacks, and LangSmithDeploymentTransport are not shipped in the KMP runtime; Kotlin callers use framework-neutral ModelMessage/UIMessage/Flow APIs instead'],
  ['@ai-sdk/llamaindex', 'not-ported: the LlamaIndex JS response bridge is not shipped; no LlamaIndexEngineResponse public API exists in the KMP runtime'],
  ...
  ['@ai-sdk/test-server', 'test-only: createTestServer, UrlResponse, UrlHandler, and related Ktor MockEngine helpers live under commonTest for provider tests and are not part of the shipped runtime API'],
  ...
  ['@ai-sdk/valibot', 'not-applicable: Valibot is a TypeScript validation library; no valibotSchema public API is shipped, and Kotlin schema work uses kotlinx.serialization and the SDK Schema/Output surfaces'],
